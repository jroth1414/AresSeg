...
def render(hyp_cfg: dict, data_cfg: dict) -> str:
    ...
    lines += [
        ...
        "zero-shot arm is scored with the **region-oracle assignment**: each SAM region takes the",
        "majority ground-truth class among its valid (non-ignore) pixels; pixels outside every",
        "proposal remain unlabeled and score as wrong for the covered classes. This is an EXPLICIT",
        "UPPER BOUND on any zero-shot region labeler built on SAM's masks, and is reported as such.",
        "",
